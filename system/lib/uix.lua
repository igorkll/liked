...
local function checkZone(self, eventData)
    return eventData[3] >= self.x and eventData[4] >= self.y and eventData[3] < self.x + self.sx and
    eventData[4] < self.y + self.sy
end
                ...
                if self.type == "context" then
                    ...
                    if not self.th then
                        self.th = thread.create(function()
                            ...
                            local px, py, sx, sy = gui.contextPos(self.gui.window.screen, x, y,
                                gui.contextStrs(self.strs))
...
function objclass:draw()
    ...
    elseif self.type == "big_switch" then
        ...
        local x, y = self.gui.window:toRealPos(self.x, self.y)
        image.draw(self.gui.window.screen,
            self.state and "/system/images/switch_on.t2p" or "/system/images/switch_off.t2p", x, y, true)
    ...
    elseif self.type == "seek" then
        ...
        if self.vertical then
            ...
            self.gui.window:fill(self.x + dotpos, self.y, self.size - dotpos, 1, bg, self.color,
                gui_container.chars.wideSplitLine)
            if style == "round" then
    ...
    elseif self.type == "up" then
        liked.drawFullUpBar(self.gui.window.screen,
            (self.gui.returnLayout and string.rep(" ", #self.gui.returnTitle) or "") .. self.title, self.withoutFill,
            self.bgcolor, self.wide, true)
        if self.gui.returnLayout then
    ...
    elseif self.type == "image" then
        local x, y = self.gui.window:toRealPos(self.x, self.y)
        image.draw(self.gui.window.screen, self.path, x, y, self.wallpaperMode, self.forceFullColor, self.lightMul,
            self.imagePaletteUsed, self.blackListedColor, self.newColors)
...
function uix:center(offsetX, offsetY, sizeX, sizeY, ...)
    local x, y = math.round((self.window.sizeX / 2) - (sizeX / 2)) + offsetX + 1,
        math.round((self.window.sizeY / 2) - (sizeY / 2)) + offsetY + 1
    return x, y, sizeX, sizeY, ...
end

function uix:centerOneSize(offsetX, offsetY, sizeX, ...)
    local x, y = math.round((self.window.sizeX / 2) - (sizeX / 2)) + offsetX + 1,
        math.round(self.window.sizeY / 2) + offsetY + 1
    return x, y, sizeX, ...
end
...
function uix.regDrawZone(self, sizeX, sizeY)
    return graphic.createWindow(self.gui.screen, self.gui.window.x + (self.x - 1), self.gui.window.y + (self.y - 1),
        sizeX, sizeY)
end
